...
def feature_extract(url_input):
        ...
        Feature['URL']= url_input   ##url

        ##通过端口返回域名和国家信息
        Feature['rank_host'],Feature['rank_country'] = sitepopularity(host)

        Feature['host']=obj.netloc   ##域名
        Feature['path']=obj.path    ##路径

        Feature['Length_of_url'] = len(url_input)   ###链接长度
        Feature['Length_of_host'] = len(host)       ###域名长度
        Feature['No_of_dots']=url_input.count('.')  ###点的数量
