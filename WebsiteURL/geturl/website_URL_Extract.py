...
def inputurl_check(url):
    try:
        ...
        return inputurl_check(website_url)
...
# 去除不是该站点的url，并且去除重复的链接信息   ##暂时不用
def url_filtrate1(dns, pagelinks):
    website_url = []
    # url_begin = "http://tomato.groov.pl/download"
    for link in pagelinks:
        # if re.findall(url_begin,link):    ###针对tamoto处理的
...
###使用相似度计算，判断是属于同一域名。并且去除重复的链接信息
def url_filtrate(urlprotocol,dns, pagelinks):
    website_url = []
    for link in pagelinks:
        url_split = urlparse.urlparse(link)
        pagelinks_urlprotocol = url_split.scheme
        pagelinks_dns = url_split.netloc
        sim = difflib.SequenceMatcher(None, dns, pagelinks_dns).ratio()    ###计算相似度
...
###写入已经获取的链接
def write_right_links(link):
    try:
        with open('url_spider_vendorname.txt', 'a') as f:
            f.write(link)
            f.write('\n')
            f.close()
    except Exception, e:
...
class URLSpider():
    ...
    def pageCrawler(self):
        while not self.linkQuence.unvisitedurlsempty():   # 若未访问队列非空
            ...
            right_links = url_filtrate(self.urlprotocol, self.dns, pagelinks)   # 筛选出合格的链接
            if right_links:
                right_url = right_links
                print right_url
                for link in right_url:  # 将筛选出的链接放到未访问队列中
                    write_right_links(link)
